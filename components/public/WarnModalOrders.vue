  ...
  <div>
      ...
      <div @click.self="closeModal" class="modal-container">
        <div class="modal-body w-50 w-sm-75">
          ...
          <div class="text-center">
            <h1>آیا شما مطمئن هستید که میخواهید این سفارش را حذف کنید؟</h1>
          </div>
    ...
    <div class="myModal" v-show="showModal2">
      <div class="succussNote">
        <div class="succussBody">سفارش های کاربر شما با موفقیت حذف شد.</div>
      </div>
    </div>
  </div>
</template>
      ...
      try {
        for (let i = 0; i < this.userId.length; i++) {
          const res = await this.$axios.$delete(
            `managers/api/orders/delete/${this.userId[i].id}/`
          )
          console.log(res)
        }
