  ...
  computed: {
    filteredUserNames() {
            ...
            if (filteredName.length > 3)
                filteredName = filteredName.slice(0, 3);
            return this.users.filter(user => user.first_name.includes(filteredName) || user.last_name.includes(filteredName));
        },
      filteredCountry() {
          if(!this.countryFilter && this.userNamesFilter) return []
          let filteredCountry = this.countryFilter
          if (filteredCountry.length > 3) filteredCountry = filteredCountry.slice(0, 3)
