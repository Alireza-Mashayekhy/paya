...
export default {
  ...
  async asyncData({ $axios }) {
    try {
      ...
      const notification = await $axios.$get(`experts/api/notification/see/`)
  ...
  methods: {
    async onSubmit() {
      if (this.newPassAgain === this.newPass) {
        try {
          const res = await this.$axios.$patch(`experts/api/account/update/`, {
    ...
    async onChangeNotifiction(notification) {
      try {
        const res = await this.$axios.$post(
          `experts/api/notification/change/`,
          {
            notification,
          }
        )
